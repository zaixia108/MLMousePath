...
def mouse_click(event):
    ...
    if event.x >= ball1_pos[0] - ball_radius and event.x <= ball1_pos[0] + ball_radius and event.y >= ball1_pos[
        1] - ball_radius and event.y <= ball1_pos[1] + ball_radius:
        recording = True
        mouse_path = [(event.x, event.y)]
    elif event.x >= ball2_pos[0] - ball_radius and event.x <= ball2_pos[0] + ball_radius and event.y >= ball2_pos[
        1] - ball_radius and event.y <= ball2_pos[1] + ball_radius:
        recording = False
        canvas.delete("ball2")
        ...
        # 绘制新的第二个小球
        canvas.create_oval(ball2_pos[0] - ball_radius, ball2_pos[1] - ball_radius, ball2_pos[0] + ball_radius,
                           ball2_pos[1] + ball_radius, fill="blue", tags="ball2")
...
def save_to_csv(path):
    ...
    with open(csv_file_path, mode='a', newline='') as csv_file:
        ...
        # 写入一行数据
        csv_writer.writerow(
            [f"{key_points_x[-1]},{key_points_y[-1]}"] + [f"{key_points_x[i]},{key_points_y[i]}" for i in range(0, 10)])
...
def visualize_path(path):
    ...
    plt.text(key_points_x[-1], key_points_y[-1], f'Distance to Origin: {key_points_distances[-1]:.2f}', ha='right',
...
canvas.pack()
canvas.create_oval(ball1_pos[0] - ball_radius, ball1_pos[1] - ball_radius, ball1_pos[0] + ball_radius,
                   ball1_pos[1] + ball_radius, fill="red")
canvas.create_oval(ball2_pos[0] - ball_radius, ball2_pos[1] - ball_radius, ball2_pos[0] + ball_radius,
                   ball2_pos[1] + ball_radius, fill="blue", tags="ball2")
